...
class TalkingPointField(BaseModel):
    ...
    class Config:
        # Allow extra fields for backward compatibility with older clients
        # (some clients send `title` instead of `label` or `value`). Do not
        # raise on unknown keys here; validation logic below will map legacy
        # names to the canonical fields.
        extra = "ignore"
        ...
        try:
            for section_idx, section in enumerate(talking_points):
                ...
                for field_idx, field in enumerate(fields):
                    ...
                    # Support legacy clients that used `title` as a field key.
                    # Prefer explicit `label`/`value` when provided, otherwise
                    # fall back to `title` so older payloads still work.
                    ...
                    # Validate field name; if missing, try to derive a sensible
                    # fallback from the title/label so legacy payloads with no
                    # explicit name still yield a usable field.
                    if not field_name:
                        fallback_name = (field_label or field.get("value") or field.get("title") or "")
                        if fallback_name:
                    ...
                    # If the client supplied a legacy `title` payload and no
                    # explicit `value`, use it as the field value fallback.
