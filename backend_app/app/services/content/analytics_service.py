...
class AnalyticsService:
    ...
    async def track_user_session(
        ...
        """Track a user session related action (e.g., login, logout) in sessions container.

        This provides backward compatibility for existing routers that call
        analytics_service.track_user_session(). The method creates a lightweight
        session activity record (not a full session heartbeat record which is
        handled by middleware) so historical login/logout style events can be
        queried without overloading the events container semantics.

        Args:
            user_id: The user performing the action.
            action: Action string such as 'login', 'logout'.
            metadata: Optional additional context.

        Returns:
            The created activity ID or empty string on failure.
        """
        try:
            if not user_id or not action:
                self.logger.error("track_user_session called without required params")
                return ""

            if not hasattr(self.cosmos_db, 'sessions_container') or self.cosmos_db.sessions_container is None:
                # Graceful degrade – don't raise, just log
                self.logger.warning("Sessions container unavailable; cannot record session activity")
                return ""
            ...
            self.cosmos_db.sessions_container.create_item(body=item)
            self.logger.info(f"✓ Session activity tracked: {action} for user {user_id} ({activity_id})")
            return activity_id
        except Exception as e:
            self.logger.warning(f"Failed to track session activity {action} for user {user_id}: {e}")
            return ""
