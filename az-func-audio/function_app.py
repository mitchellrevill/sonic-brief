...
# Service modules (and constants like SYSTEM_GENERATED_TAG) are imported lazily
# inside functions to avoid import-time failures when native extensions
# (for example, cryptography used by azure.identity) are not available on the
# Functions worker during startup.
...
# Defensive: ensure the directory containing this file (and common sibling folders)
# are on sys.path so imports like `from services...` work regardless of how the
# function app was packaged or deployed (some deployment flows place files in a
# nested folder). We insert candidate paths at the front of sys.path.
base_dir = os.path.dirname(__file__)
if base_dir and base_dir not in sys.path:
    sys.path.insert(0, base_dir)
# Also accept a nested az-func-audio folder (common when building from repo root)
alt_dir = os.path.join(base_dir, "az-func-audio")
...
def refine_analysis_http(req: func.HttpRequest) -> func.HttpResponse:
    ...
    Services are imported lazily to avoid importing heavy native deps during
    Functions host indexing.
    """
    ...
    try:
        ...
        # Initialize services (lazy import)
        config = AppConfig()
...
"""
        ...
        system_context = (
            "You are an AI assistant helping to refine and improve analysis of conversations. "
            "Provide helpful, accurate refinements based on user requests."
        )
        ...
        if result.get("status") == "success":
            ...
            response_data = {
                "refined_analysis": (
                    f"I apologize, but I encountered an error while processing your refinement request: {result.get('error', 'Unknown error')}"
                ),
                "status": "error",
        ...
        return func.HttpResponse(
            json.dumps(response_data), status_code=200, headers={"Content-Type": "application/json"}
        )

    except Exception as e:
        logging.error(f"Analysis refinement failed: {str(e)}", exc_info=True)
        error_response = {
        ...
        return func.HttpResponse(
            json.dumps(error_response), status_code=500, headers={"Content-Type": "application/json"}
        )


def get_system_generated_tag() -> str:
    """Lazily return the SYSTEM_GENERATED_TAG constant from file_processing_service.

    If the import fails during indexing, return a safe default to avoid crashing.
    """
    try:
...
def blob_trigger(myblob: func.InputStream):
    ...
    try:
        # Initialize config and lazily-import services to avoid heavy imports
        ...
        # Lazily import heavy service modules here
        ...
        if file_type == "audio":
            ...
            processed_text_blob_url = storage_service.upload_text(
                ...
                blob_name=f"{path_without_container}_{tag}_processed_text.txt",
...
def process_audio_file(config, blob_url, path_without_container, job_id, cosmos_service, storage_service):
    ...
    # Lazily import the TranscriptionService to avoid importing heavy deps at module import time
    ...
    transcription_blob_url = storage_service.upload_text(
        container_name=config.storage_recordings_container,
        blob_name=f"{path_without_container}_{tag}_transcription.txt",
        text_content=formatted_text,
    )
...
# Timer-triggered session cleanup registration (must be at module level)
from services import session_cleanup

@app.function_name(name="SessionCleanupTimer")
@app.schedule(schedule="0 */5 * * * *", arg_name="mytimer", run_on_startup=False, use_monitor=True)
def session_cleanup_timer(mytimer: func.TimerRequest) -> None:
    """Timer trigger to auto-close stale sessions every 5 minutes."""
    session_cleanup.main(mytimer)
